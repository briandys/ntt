<?php
// header.php
function ntt_after_entity_primary_nav_wp_hook() {
    do_action( 'ntt_after_entity_primary_nav_wp_hook' );
}

// footer.php
function ntt_after_entity_footer_aside_wp_hook() {
    do_action( 'ntt_after_entity_footer_aside_wp_hook' );
}

// content.php
function ntt_after_entry_content_wp_hook() {
    do_action( 'ntt_after_entry_content_wp_hook' );
}

// content.php
function ntt_after_entry_name_wp_hook() {
    do_action( 'ntt_after_entry_name_wp_hook' );
}

// content.php
function ntt_after_entry_primary_meta_wp_hook() {
    do_action( 'ntt_after_entry_primary_meta_wp_hook' );
}

// content.php
function ntt_comments_css_wp_hook() {
    do_action( 'ntt_comments_css_wp_hook' );
}

// content.php
function ntt_after_the_content_wp_hook() {
    do_action( 'ntt_after_the_content_wp_hook' );
}

// entry-navigation.php
function ntt_after_entry_nav_wp_hook() {
    do_action( 'ntt_after_entry_nav_wp_hook' );
}