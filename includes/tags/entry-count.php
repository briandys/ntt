 ...
 * Entry Count
 * 
 * Displays the number of entries in a given query
 */
function ntt_entry_count( $args, $entry_count_name, $entry_count_css ) {
    ...
    if ( $query_found_posts == 0 ) {
        $entry_count_glabel = $entry_text;
    } elseif ( $query_found_posts == 1 ) {
        $entry_count_glabel = $entry_text;
    } else {
        $entry_count_glabel = $entries_text;
    }
    ?>
    <div class="<?php echo $entry_count_css; ?>-entry-count count obj" data-name="<?php echo $entry_count_name; ?> Entry Count">
        <span class="l">
